...
import { FaLinkedin, FaGithub, FaTwitter, FaMedium, FaSun, FaMoon } from "react-icons/fa";

const Navbar = ({ toggleTheme, isDarkMode }) => {
  return (
    <nav className={`mb-15 py-6 ${isDarkMode ? 'text-darkText' : 'text-lightText'}`}>
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3 m-8 text-2xl">
          <a href="https://www.linkedin.com/in/ffferchavez" target="_blank" rel="noopener noreferrer">
            <FaLinkedin className={`${isDarkMode ? 'text-darkText' : 'text-lightText'}`} />
          </a>
          <a href="https://github.com/ffferchavez" target="_blank" rel="noopener noreferrer">
            <FaGithub className={`${isDarkMode ? 'text-darkText' : 'text-lightText'}`} />
          </a>
          <a href="https://x.com/ffferchavez" target="_blank" rel="noopener noreferrer">
            <FaTwitter className={`${isDarkMode ? 'text-darkText' : 'text-lightText'}`} />
          </a>
          <a href="https://medium.com/@ffferchavez" target="_blank" rel="noopener noreferrer">
            <FaMedium className={`${isDarkMode ? 'text-darkText' : 'text-lightText'}`} />
          </a>
        </div>
      </div>
      ...
      <div className="flex justify-end mt-2 m-8">
        <button
          onClick={toggleTheme}
          className={`flex items-center text-xs rounded transition-colors duration-300 ease-in-out ${isDarkMode ? 'text-darkText' : 'text-lightText'}`}
          aria-label="Toggle theme"
        >
          {isDarkMode ? <FaMoon className="text-md mr-2" /> : <FaSun className="text-md mr-2" />}
